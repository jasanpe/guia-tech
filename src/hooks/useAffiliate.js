...
export function useAffiliate() {
    ...
    (productId, storeId, options = {}) => {
      try {
        ...
        analytics.trackEvent('affiliate_link_generated', {
          ...
          conversionPotential: options.price
            ? (options.price * AffiliateManager.getCommissionRate(storeId, options.category)) / 100
            : null,
          ...options,
        });
    ...
    async (productId, storeId, data = {}) => {
      try {
        ...
        const clickData = AffiliateManager.trackClick(productId, storeId, {
          ...
          priceHistory: priceData?.history, // Asegúrate de que esto es correcto
  ...
  const calculatePriceScore = useCallback((priceData) => {
    if (!priceData || !priceData.history) return null; // <-- Verificación crucial

    const currentPrice = priceData.currentPrice;
    const lowestPrice = priceData.history.reduce(
      (min, p) => Math.min(min, p.price),
      Infinity
    );
    const averagePrice =
      priceData.history.reduce((sum, p) => sum + p.price, 0) /
  ...
  const calculatePurchaseIntent = useCallback((clickData) => {
    let score = 0;

    // Factores que aumentan la intención
    if (clickData.priceAtClick < (clickData.priceHistory?.average || 0))
      score += 2;
