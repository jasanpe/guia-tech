...
const Article = ({ article, loading }) => (
  <article className={`bg-white shadow-lg rounded-lg overflow-hidden hover:shadow-xl transition-all transform hover:-translate-y-1 ${
    loading ? 'animate-pulse' : ''
  }`}>
    <div className="p-6">
      <span className="inline-block px-2 py-1 text-sm text-blue-600 bg-blue-50 rounded font-geist-sans uppercase">
        {article?.type || 'Artículo'}
      </span>
      ...
      <p className="text-gray-600 mb-4 text-sm font-geist-sans line-clamp-3">
        {article?.excerpt || 'Contenido no disponible'}
      </p>

      <div className="flex justify-between items-center">
        <time className="text-sm text-gray-500 font-geist-sans">
          {article?.date || 'Fecha no disponible'}
        </time>
        <Link
          href={article?.url || '/'}
...
export default function LatestArticles() {
  ...
  return (
    <section
      id="latest-articles"
      className={`my-12 transition-opacity duration-500 ${isVisible ? 'opacity-100' : 'opacity-0'}`}
    >
